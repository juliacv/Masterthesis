...
with open(f"/Home/siv31/jng001/Master_Julia/yolov5/{run_dir}/detect/detect_info.txt") as exp_file:
...
#MÅ ENDRE
with open(f"/Home/siv31/jng001/Master_Julia/yolov5/{run_dir}/val_output.txt") as val_file:
    for count,line in enumerate(val_file):
        line = line.replace("\n","").replace(" ","")
        if count == 1:
            new_line = line.replace("\n","").split(",")
            print(new_line)
        ...
        if line == f"yolov5/{run_dir}/train/{exp}//weights/best.pt":
            count += 1
        else:
            print(line)
            continue
...
info_file = open(f"/Home/siv31/jng001/Master_Julia/yolov5/{run_dir}/Yolov5_results_val.txt", "w")
